...
from utils.bbox import bbox_overlaps_batch , bbox_transform_batch
...
class _AnchorTargetLayer(nn.Module):
    ...
    def forward(self, rpn_cls_score,gt_boxes,im_info,rpn_cls_score_OHEM = None):
        ...
        shifts = shifts.contiguous().type_as(rpn_cls_score).double()
        ...
        keep = ((all_anchors[:, 0] >= -self._allowed_border) &
                ...
                (all_anchors[:, 2] < int(im_info[1]) + self._allowed_border) & # width
                (all_anchors[:, 3] < int(im_info[0]) + self._allowed_border))  # height
        ...
        # return outputs
        return labels , bbox_targets.float(), bbox_inside_weights.float(), bbox_outside_weights.float()
