...
class CityPersons(Dataset):
    def __init__(self, path, type, config, preloaded=False, transform=None, caffemodel=False):
        ...
        self.dataset = get_citypersons(root_dir=path, type=type, num_classes=config.num_classes)
        ...
        if self.type == 'train' :
            ...
            self.preprocess = RandomResizeFix(size=config.size_train)
            self.valsize = config.size_train
        else:
        ...
        if self.preloaded:
            ...
            for i, data in enumerate(self.dataset):
                if self.caffemodel:
                    ...
                    self.img_cache.append(Image.open(data['filepath']).convert('RGB'))
                print('%d/%d\r' % (i+1, self.dataset_len)),
                sys.stdout.flush()
            #print(self.img_cache)
            print('')
    ...
    def __getitem__(self, item):
        ...
        if self.caffemodel:
            ...
            if self.type == 'train':
                ...
                labels = img_data['label'].copy()

                y_center, y_height, y_offset = self.calc_gt_center(gts, igs, labels, radius=2, stride=self.config.down, mode=self.type)

                x_img = x_img.astype(np.float32)
                x_img -= [103.939, 116.779, 123.68]
                x_img = torch.from_numpy(x_img).permute([2, 0, 1])

                return x_img, [y_center, y_height, y_offset], gts
            ...
            if self.type == 'train' or 'val':
                ...
                if self.type == 'train':
                    ...
                    x_img = img.resize((self.valsize[1], self.valsize[0]))
                else:
                    x_img = img
                ...
                if len(boxes) > 0:
                    ...
                    cv2.imwrite('./data/output/detection/train_check.jpg', im)
                    print('make image')
                ...
                return x_img, boxes, [y_center, y_height, y_offset]
    ...
    def calc_gt_center(self, gts, igs, cls, radius=2, stride=4, mode='train'):
        ...
        if len(gts) > 0:
            ...
            for ind in range(len(gts)):
                ...
                pos_map[1+boxcls, c_y, c_x] = 1 # center map
...
class RandomResizeFix(object):
    ...
    def __call__(self, img, gts, igs):
        ...
        if h >= self.size[0]:
            # random crop
            img, gts, igs = self.random_crop(img, gts, igs, self.size, limit=8)
        else:
            # random pad
