...
def get_citypersons(root_dir='data/cityperson', type='train', num_classes = 1):
    ...
    for l in range(len(annos[index][0])):
        ...
        for i in range(len(gts)):
            ...
            if label <= num_classes and h >= 25 and label > 0:
                box = np.array([int(x1), int(y1), int(x1) + int(w), int(y1) + int(h), label])
                boxes.append(box)
                labels.append(label)
                vis_box = np.array([int(xv1), int(yv1), int(xv1) + int(wv), int(yv1) + int(hv), label])
